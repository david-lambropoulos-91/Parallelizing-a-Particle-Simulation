...
{    
	...
	first_real_bin *= binsPerRow;
			...
			for ( int p = 0; p < size; ) 
			{
				...
				if ( loc != biter )
                    			...
                    			if (loc < ob1 || loc > ob2 )
					{
                        			if( zip > n / 2 )
						{
                            				fprintf( stderr, "Error, buffer overflow localzip at line %d in file %s\n", __LINE__, __FILE__ );
        	...
        	for( int i = 0; i < numProc; i++ )
		{
			...
			{
                    		fprintf( stderr, "Error, buffer overflow at line %d in file %s\n", __LINE__, __FILE__ );
	        ...
	        for( int i = 0; i < fa; i++ )
		{
			...
			if ( loc < ob1 || loc > ob2 )
			{
                		fprintf(stderr, "rank %d out of bounds0 at line %d in file %s\n", rank, __LINE__, __FILE__ );
			}
            		else
			{
		...
		for( int i = 0; i < fb; i++ )
		{
			...
			if ( loc < ob1 || loc > ob2 )
			{
                		fprintf(stderr, "rank %d out of bounds1 at line %d in file %s\n", rank, __LINE__, __FILE__);
			}
            		else
			{
        	...
        	if( rank > 0 )
		{
            		for ( int j = 0; j < fa; j++ ) 
			{
                		...
                		if ( loc < ob1 || loc > ob2 )
				{
                    			fprintf(stderr, "rank %d out of bounds2 at line %d in file %s\n", rank, __LINE__, __FILE__);
				}
                		else
				{
        	...
        	if( rank < numProc - 1 )
		{
            		for ( int j = 0; j < fb; j++ ) 
			{
                		...
                		if ( loc < ob1 || loc > ob2 )
				{
                    			fprintf(stderr, "rank %d out of bounds3 at line %d in file %s\n", rank, __LINE__, __FILE__);
				}
                		else
				{
        	...
        	for( int j = 0; j < tempmovesize; j++ )
		{
			...
			if ( loc < ob1 || loc > ob2 )
                	{
				fprintf(stderr, "rank %d out of bounds4 at line %d in file %s\n", rank, __LINE__, __FILE__ );
			}
            		else
            		{
